<template>
  <div class="retreat">
    <Header :text="'退钟'" :handle="true"  @back="backPage" >
    </Header>
    <div class="content">
      <div class="infos">
        <div class="tips">是否确认对{{updateData.TechnicianNo}}技师进行退钟?</div>
      </div>
      <div class="btn-group">
        <mt-button size="large" class="btn confirm-btn"  @click.native="repeatClock" >确定</mt-button>
...
<script>
  export default ({
    ...
    methods:{
      backPage:function () {
        this.$router.push({'path':'/manageTech',query:this.updateData})
      },
      repeatClock:function () {
        ...
        ts.$api.post('RetreatTask',params).then(function (rets) {
          console.log(rets)
          if(rets.code === 0){
